...
if (urlSearch.indexOf("/edit/") >= 0)
{
	...
	$( document ).ready(function() {
		...
		$('.content-inner').append('<div id="previewBtn">' + '<img src="' + src + '" alt="Test Case Preview" title="Test Case Preview" height="32" width="32" />' + '</div>');
		$('#previewBtn').click(function(){
			var notes = $('#custom_notes').val();
			var preconditions = $('#custom_preconds').val();
			var steps = "<table><tr><th></th><th style='border-bottom: 1px solid black'>Step</th><th style='border-bottom: 1px solid black'>Expected Result</th></tr>";
			$('.steps.steps-control').children('tbody').children('tr').each(function( index ) {
				...
				steps = steps + "<tr><td>"+(index+1)+".</td><td style='border-bottom: 1px solid black'>"+micromarkdown.parse(step)+"</td><td style='border-bottom: 1px solid black'>"+micromarkdown.parse(expected)+"</td></tr>";
			});
			steps = steps + "</table>";

			$('body').append("<div id='toNewWindow'>\
				Notes<hr>"
			...
			var html = $("#toNewWindow").html();
		  	html = html + '<p><input type="button" value="Close preview" id="close" onclick="window.close()" /></p>';

		  	//Check if browser is Chrome
		    if (navigator.userAgent.indexOf("Chrome") >= 0){
		        var w = window.open();
		        w.document.title = "Test Case Preview";
		    ...
		    //Check if browser is Firefox 
		    	...
		    	alert("Unsupported browser, please use FireFox or Chrome");
		    }
